...
function ocultar() {
    ...
    if (car_items.childElementCount == 0) {
...
function agregaritemalcar(titulo, precio, imagenSrc) {
    ...
    // Creamos el div y le añadimos la clase directamente aquí para no colocarla luego
    var item = document.createElement('div');
    item.classList.add('car-item');

    var itemscar = document.getElementsByClassName('car-items')[0];
    ...
    for (var i = 0; i < nombre_itemscar.length; i++) {
        ...
        // Aquí solo colocaremos el contenido del contendor del producto
        var itemscar_conte = `
        ...
        // Insertamos el contenido al cotenedor del producto
        item.innerHTML = itemscar_conte;


        // Con esta opción insertas siempre arriba del todo del elemento padre
        itemscar.insertAdjacentElement("afterbegin", item);

        // Con esta opción insertas una un elemento de penultimo, es decir antes del último elemento padre
        // itemscar.insertBefore(item, itemscar.lastElementChild);

        // Aqui lo que hacemos es actualizar el carrito con el producto que acabamos de añadir
        actualizar_totalcar();

        // Y con esto asiganmos todas las funciones de sumar, restar y eliminar productos
        ready();
}